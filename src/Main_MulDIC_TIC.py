...
class EmbeddingLayer:
    def __init__(self, project, wordSet, embeddingSize, data_type):
        ...
        if data_type == 'text':
            try:
                self.embedder = torch.load('./EmbeddingModel/{}-EmbeddingLayer_down_Mul_T-I-C_text.pt'.format(project))
            except FileNotFoundError:
                self.embedder = nn.Embedding(num_embeddings=len(wordSet), embedding_dim=embeddingSize)
                torch.save(self.embedder, './EmbeddingModel/{}-EmbeddingLayer_down_Mul_T-I-C_text.pt'.format(project))
        elif data_type == 'code':
            try:                
                self.embedder = torch.load('./EmbeddingModel/{}-EmbeddingLayer_down_Mul_T-I-C_code.pt'.format(project))
            except FileNotFoundError:
                self.embedder = nn.Embedding(num_embeddings=len(wordSet), embedding_dim=embeddingSize)
                torch.save(self.embedder, './EmbeddingModel/{}-EmbeddingLayer_down_Mul_T-I-C_code.pt'.format(project))
...
class MAIN:
    ...
    def run(self):
        ...
        modelPath = './model_vscode_MulDIC_TIC/'
        # modelPath = './model_kubernetes_MulDIC_TIC/'
        # modelPath = './model_flutter_MulDIC_TIC/'
        # modelPath = './model_roslyn_MulDIC_TIC/'
